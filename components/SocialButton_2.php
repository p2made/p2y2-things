...
class SocialButton extends \p2made\components\base\P2ComponentBase
{
	...
	{
		$this->service = $this->_services[$name];
		$this->caption = $this->serviceIcon()
			. $this->defaultCaption . $this->service['name'];

		Html::addCssClass($options, BSocial::$cssPrefix . ' btn-block btn-social-icon');
		Html::addCssClass($options, BSocial::$cssPrefix . '-' . $name);
	...
	public function caption($value = null)
	{
		...
		$caption = FA::icon($this->service['icon']);
		$caption = $caption . str_replace('@@@', $this->service['name'], $value);
	...
	protected function serviceIcon()
	{
		return FA::icon($this->service['icon']);
	}

	protected static function getCaption()
	{
		//$caption = $this->serviceIcon() . $this->defaultCaption . $this->service['name'];
	}
