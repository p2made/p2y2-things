...
```
	p2m\assets\BootstrapIcons::register($this);
```

or specify as a dependency in another asset bundle loaded to the same page with...

```
	'p2m\assets\BootstrapIcons',
```

Also include `use p2m\icons\BI;` at the top of the page.

Now your page is ready for Bootstrap Icons!
...
* `static BI::icon($name, $options=[])` - Creates an [`Icon`](#class-componenticon) that can be used to insert a Bootstrap Icons html icon.
	* `$name` - name of icon in Bootstrap Icons set.
	* `$options` - additional attributes for `i.bi` html tag.
* `static BI::i($name, $options=[])` - Shortcut for `BI::icon()`.
